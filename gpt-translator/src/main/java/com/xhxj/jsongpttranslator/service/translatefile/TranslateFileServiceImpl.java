...
public class TranslateFileServiceImpl extends ServiceImpl<TranslateFileMapper, TranslateFile> implements TranslateFileService {
    ...
    public List<TranslateFile> fileListByProjectId(Long projectId) {
        ...
        translateFiles.forEach(v -> {
            v.setCompleted(translationDataService.count(new LambdaQueryWrapper<TranslationData>().eq(TranslationData::getFileId, v.getFileId()).isNotNull(TranslationData::getTranslationText)));
            v.setNotCompleted(translationDataService.count(new LambdaQueryWrapper<TranslationData>().eq(TranslationData::getFileId, v.getFileId()).isNull(TranslationData::getTranslationText)));
        });
